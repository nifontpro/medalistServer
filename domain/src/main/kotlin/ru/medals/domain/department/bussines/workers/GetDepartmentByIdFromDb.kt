...
fun ICorChainDsl<DepartmentContext>.getDepartmentByIdFromDb(title: String) = worker {
	...
	handle {
		department = departmentRepository.getDepartmentById(departmentIdValid) ?: run {
			fail(
				errorDb(
					repository = "department",
		...
		companyIdValid = department.companyId
	}
}